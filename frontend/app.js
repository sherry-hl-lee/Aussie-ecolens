...
function clearLocalAuth() {
  ...
  updateAuthStatus();
  el('tokenHint').textContent = 'Signed out. Use Login to sign in again with Cognito Hosted UI.';
}
...
async function refreshAuthState() {
  try {
    ...
    el('tokenHint').textContent =
      'ID token loaded from Amplify session (used for API Bearer auth).';
  } catch {
    updateAuthStatus();
  }
}
...
async function verifyToken() {
  try {
    ...
    if (!state.token) {
      throw new Error('No Cognito session. Click Login first.');
    }
